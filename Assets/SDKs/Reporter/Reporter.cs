...
public class Reporter : MonoBehaviour
{
    ...
    public void Initialize()
    {
        ...
        SceneManager.sceneLoaded += this._onSceneLoaded;
    }
    ...
    void Start()
    {
        ...
        if (IsLogSaved)
        {
            ...
            string _name = System.DateTime.Now.Year.ToString() + System.DateTime.Now.Month.ToString().PadLeft(2, '0') + System.DateTime.Now.Day.ToString().PadLeft(2, '0') + System.DateTime.Now.Hour.ToString().PadLeft(2, '0') + System.DateTime.Now.Minute.ToString().PadLeft(2, '0') +
                           System.DateTime.Now.Second.ToString().PadLeft(2, '0');
            outpath = Application.persistentDataPath + "/" + _name + ".txt";
            System.IO.DirectoryInfo dir = new DirectoryInfo(Application.persistentDataPath);
            if (dir.Exists)
            {
                ...
                if (fileList.Length > 3)
                {
                    ...
                    long[] names = new long[fileList.Length];
                    for (int i = 0; i < fileList.Count(); i++)
                    {
                        string pStrName = fileList[i].Name.Substring(0, fileList[i].Name.Length - 4);
                        names[i] = Convert.ToInt64(pStrName);
                    }
                    ...
                    for (int i = 0; i < names.Length; i++)
                    {
                        if (i > 3)
                        {
                            string _path = Application.persistentDataPath + "/" + names[i] + ".txt";
                            //每次启动客户端删除之前保存的Log
