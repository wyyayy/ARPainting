...
namespace ProtoBuf.Meta
{
    ...
    public class MetaType : ISerializerProxy
    {
                ...
                foreach (MemberInfo member in foundList)
            {
                ...
                if((property = member as PropertyInfo) != null)
                {
                    ...
                    if (!property.CanWrite)
                    {
                        ...
                        var template = "<{}>k__BackingField";
                        var backingFieldName = string.Format(template, property.Name);
        ...
        private ValueMember AddField(int fieldNumber, string memberName, Type itemType, Type defaultType, object defaultValue)
        {
            ...
            var temp = mi as PropertyInfo;

            if (temp != null && temp.CanWrite == false)
            {
                string template = "<{}>k__BackingField";
                string name = string.Format(template, ((PropertyInfo)mi).Name);

                var backingMembers = type.GetMember(name, Helpers.IsEnum(type) ? BindingFlags.Static | BindingFlags.Public : BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
